...
export async function createDeliveryOrder(order: DeliveryOrder): Promise<KoombiyoApiResponse> {
	const orderData: Record<string, string> = {
		orderWaybillid: order.orderWaybillid,
		...
		spclNote: order.spclNote,
		getCod: order.getCod,
	};
	return makeKoombiyoRequest('/Addorders/users', orderData);
}

export async function createPickupRequest(pickup: PickupRequest): Promise<KoombiyoApiResponse> {
	const pickupData: Record<string, string> = {
		vehicleType: pickup.vehicleType,
		pickup_remark: pickup.pickup_remark,
	...
	return makeKoombiyoRequest('/Pickups/users', pickupData);
}
